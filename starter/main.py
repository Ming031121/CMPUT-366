...
class MRV(VarSelector):
    ...
    def select_variable(self, grid):
        ...
        for i in range(9):
            for j in range(9):
                if 1 < len(grid.get_cells()[i][j]) < sl:
                    minimum = (i, j)
                    sl = len(grid.get_cells()[i][j])
        return minimum
...
class AC3:
    ...
    def pre_process_consistency(self, grid):
        ...
        self.consistency(grid, arcs_to_make_consistent)
    ...
    def consistency(self, grid, Q):
        ...
        #     checked.add(cell)
        #
        #     assigned_row, failure = self.remove_domain_row(grid,cell[0], cell[1])
        #     if failure: return failure
        #
        #     assigned_column, failure = self.remove_domain_column(grid,cell[0], cell[1])
        #     if failure: return failure
        #
        #     assigned_unit, failure = self.remove_domain_unit(grid,cell[0], cell[1])
        #     if failure: return failure
        #
...
for p in problems:
    ...
    # new_g = BacktrackingForward().search(g, FirstAvailable().select_variable(g))
    # t_stop = time.perf_counter()
    # print("new puzzle")
    # new_g.print()
    # print("running time for Backtracking with forward checking is:" + str(t_stop-t_start))


    # Backtracking test with MRV heuristic variable selection
    t_start = time.perf_counter()
    new_g = Backtracking().search(g, MRV().select_variable(g))
    t_stop = time.perf_counter()
    print("new puzzle")
    new_g.print()
    ...
    print('Is the current grid a solution? ', new_g.is_solved())
    print("running time for Backtracking is:" + str(t_stop-t_start))




    # # Backtracking test with Naïve method variable selection
    # new_g = Backtracking().search(g, FirstAvailable().select_variable(g))
    # print("new puzzle")
    # new_g.print()
    #
    # print('Domains of Variables')
